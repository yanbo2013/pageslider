...
;(function($, window, document, undefined) {
	...
	PageSlider.prototype.showSec = function(index, direction) {
		...
		$nextSec.addClass('next').css('-webkit-transform', this.motion(this.pageHeight)).children('img').hide();
		$prevSec.addClass('prev').css('-webkit-transform', this.motion(-this.pageHeight)).children('img').hide();

		$curSec.addClass('ani').siblings('.section').removeClass('ani');
		if(cur == 1){
			$curSec.children('img').css({animation: 'shake 0.5s linear forwards'});
			$curSec.children('img').show(500);
		}else if (cur == 2) {
		 	var btn1 =$('#btn1');
			btn1.css({animation: 'translate_kf 0.5s'});
			btn1.show(500);
			var btn2 =$('#btn2');
			btn2.css({animation: 'translate_right 0.5s'});
			btn2.show(500);
			var btn3 =$('#btn3');
			btn3.css({animation: 'translate_kf 0.5s'});
			btn3.show(500);
			var btn4 =$('#btn4');
			btn4.css({animation: 'translate_right 0.5s'});
			btn4.show(500);
		 } /*else if(cur == 4){
			$curSec.children('img').css({animation:'slidedown 0.5s'});
			$curSec.children('img').show(500);

		}*/ else{
			$curSec.children('img').css({animation: 'shake 0.5s linear forwards'});
			$curSec.children('img').show(500);
		//	$curSec.children('img').anim('shake',500,'linear');
		// 	$curSec.children('img').slideDown(500);
			// 	$curSec.children('img').animate('shake',
			// 		500,
			// 	'linear',function () {
			// 		$curSec.children('img').animate('shake','linear');
			// 	});
	...
	//.children('img')
	// 			.animate({margin:'150px 0 0 -80px',opacity:'1',rotate:'-45deg'},800,'ease-in-out',function(){})
	/**
